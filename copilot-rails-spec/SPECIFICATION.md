...
4. **As an agent user**, I want to say “Run the task *dev:all*” and be confident it won’t stomp on running tasks or busy ports.
...
- **R3 Script conflicts** → Default to *not overwriting*; `--force` + `.bak` for explicit changes.  
  ...
  "tasks": [
    {
      ...
      "problemMatcher": {
        ...
        "background": {
          "activeOnStart": true,
          "beginsPattern": "Starting|vite v\d+.*|Starting the development server",
